...
&function_begin("GFp_${PREFIX}_set_decrypt_key");
	&mov	($inp,&wparam(0));		# inp
	&lea	($base,&DWP(-56,"esp"));
	&mov	($round,&wparam(1));		# bits
	&and	($base,-16);
...
&set_label("pic_point");
	...
	&mov	("esp",&DWP(48,"esp"));
	&xor	("eax","eax");
&function_end("GFp_${PREFIX}_set_decrypt_key");
...
&function_begin("GFp_${PREFIX}_decrypt");
	&lea	($const,&DWP(&label("_vpaes_consts")."+0x30-".&label("pic_point")));
	&call	("_vpaes_preheat");
&set_label("pic_point");
	&mov	($inp,&wparam(0));		# inp
	...
	&mov	("esp",&DWP(48,"esp"));
&function_end("GFp_${PREFIX}_decrypt");
