...
.globl	GFp_${PREFIX}_set_decrypt_key
.type	GFp_${PREFIX}_set_decrypt_key,\@function,3
.align	16
GFp_${PREFIX}_set_decrypt_key:
___
$code.=<<___ if ($win64);
	lea	-0xb8(%rsp),%rsp
	movaps	%xmm6,0x10(%rsp)
...
$code.=<<___;
	xor	%eax,%eax
	ret
.size	GFp_${PREFIX}_set_decrypt_key,.-GFp_${PREFIX}_set_decrypt_key
...
.globl	GFp_${PREFIX}_decrypt
.type	GFp_${PREFIX}_decrypt,\@function,3
.align	16
GFp_${PREFIX}_decrypt:
___
$code.=<<___ if ($win64);
	lea	-0xb8(%rsp),%rsp
	movaps	%xmm6,0x10(%rsp)
...
$code.=<<___;
	ret
.size	GFp_${PREFIX}_decrypt,.-GFp_${PREFIX}_decrypt
___
$code.=<<___;
##
##  _aes_preheat
...
.align	4
	...
	.rva	.LSEH_begin_GFp_${PREFIX}_set_decrypt_key
	.rva	.LSEH_end_GFp_${PREFIX}_set_decrypt_key
	.rva	.LSEH_info_GFp_${PREFIX}_set_decrypt_key

	.rva	.LSEH_begin_GFp_${PREFIX}_encrypt
	.rva	.LSEH_end_GFp_${PREFIX}_encrypt
	.rva	.LSEH_info_GFp_${PREFIX}_encrypt

	.rva	.LSEH_begin_GFp_${PREFIX}_decrypt
	.rva	.LSEH_end_GFp_${PREFIX}_decrypt
	.rva	.LSEH_info_GFp_${PREFIX}_decrypt
...
.LSEH_info_GFp_${PREFIX}_set_decrypt_key:
	.byte	9,0,0,0
	.rva	se_handler
...
.LSEH_info_GFp_${PREFIX}_decrypt:
	.byte	9,0,0,0
	.rva	se_handler
